export default {
  async fetch(request, env) {
    ...
    // --- HÀM CHUYỂN ĐỔI JSON (Dùng cho Admin Upload Đề) ---
    function flattenQuestions(list, quizId, parentId = null, order = { v: 0 }) {
      const qRows = [], oRows = [];
      for (const q of list) {
        qRows.push({
          question_id: q.id, quiz_id: quizId, parent_id: parentId,
        ...
        if (q.type === "true_false") {
          opts = [
            { id: "true", content: q.options?.true || "Đúng", is_correct: (q.correct_answers || []).includes("true") },
            { id: "false", content: q.options?.false || "Sai", is_correct: (q.correct_answers || []).includes("false") }
          ];
        } else if (q.type === "multiple_true_false") {
    ...
    try {
      ...
      // 1. Lấy danh sách để tự dò ID (Quên đăng nhập)
      ...
      // 4. Xem BXH & Lịch sử cá nhân
      if (path === "/api/quiz/leaderboard" && method === "GET") {
        const quizId = url.searchParams.get("quiz_id");
        const { results } = await env.DB.prepare(`
          SELECT u.display_name, r.score, r.correct_count, (JULIANDAY(r.end_time) - JULIANDAY(r.start_time))*1440 as duration 
      ...
      // 3. Soi chi tiết từng câu của 1 học sinh
